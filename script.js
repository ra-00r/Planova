...
async function drawPerformanceChart(userId) {
  ...
  // Build daily task completion map
  const { data: allTasks } = await sb.from("tasks").select("*").eq("user_id", userId);
  const dailyMap = {};
  if (allTasks) {
    allTasks.forEach((t) => {
      if (!t.is_done) return;
      const day = (t.due_date || "").slice(0, 10) || new Date().toISOString().slice(0, 10);
  ...
  dailyDays.forEach((day) => {
    ...
    completionData.push(dailyMap[day]);
  ...
  if (exams && exams.length) {
    exams.forEach((ex) => {
      if (ex.score != null) {
        labels.push(ex.subject ? escapeHtml(ex.subject) : "Exam");
      ...
      datasets: [
        {
          ...
          backgroundColor: "rgba(79,141,255,0.75)",
